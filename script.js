...
function updateCesiumCamera(dt) {
    ...
    // terrain, or whenever no World Terrain is loaded). Needed for BOTH
    // ground vehicles AND the plane now: flight.alt is an above-ground-
    // level (AGL) value (it starts at 20ft "on the ground" and is floored
    // at 20ft in updateFlight()), not an absolute sea-level altitude. Using
    // it alone as the plane's height above the ellipsoid silently assumed
    // the ground was always at 0m — true on the flat EllipsoidTerrainProvider
    // (no Ion token), but as soon as real World Terrain is loaded, the
    // actual ground is almost never at exactly 0m. That's what made the
    // plane spawn "inside" the flat-map ground/sink below real hills once
    // a key was set, needing a manual climb to clear it. Sampling this
    // every frame (cheap — see getGroundHeight's docstring) and adding it
    // under flight.alt means the plane is always at least flight.alt feet
    // above whatever terrain is actually beneath it — i.e. it can no
    // longer fly or spawn below the real Cesium 3D ground.
    const groundHeight = getGroundHeight(state.lng, state.lat);
    ...
    // REAL GROUND (not sea level). The plane now uses its real, simulated
    // flight altitude ABOVE THAT SAME REAL GROUND, instead of above sea level.
    const eyeHeight = isPlane
        ? groundHeight + flight.alt * 0.3048                // real ground + ft→m AGL
    ...
    // person, as the chase camera's reference frame. Both ground vehicles
    // AND the plane are now anchored above the REAL terrain surface
    // (groundHeight, computed above) — the plane simply adds its AGL
    // altitude on top of it instead of treating sea level as the ground.
