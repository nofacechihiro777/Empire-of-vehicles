...
function buildVehicleModels() {
    ...
    Object.keys(VEHICLE_MODEL_DEFS).forEach(type => {
        ...
        if (def && def.type === 'glb') {
            ...
            // Load the GLB twice and stack both copies at the exact same
            // position.  Cesium's translucent pipeline sorts per-primitive,
            // not per-triangle, so a single copy shows see-through gaps.
            // Two overlapping render passes fill those holes: the first copy
            // seeds the depth buffer; the second resolves the back-faces that
            // the first left transparent.  updateVehicleModels() writes the
            // same modelMatrix to every entry in the array, so both copies
            // track the plane perfectly with zero extra logic.
            for (let _glbCopy = 0; _glbCopy < 2; _glbCopy++) {
