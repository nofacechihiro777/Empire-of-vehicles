...
function update() {
    ...
    // Cesium continues loading terrain in the background via its own render loop.
    if (!gameStarted) {
...
function confirmSpawnLocation() {
    ...
    // ── 5. Start the game ─────────────────────────────────────────────────
    // Small delay so the fade animation completes and Cesium has a frame
    // to position the camera before the game loop's first physics tick.
    setTimeout(() => {
        gameStarted = true;
